...
{
	if(m1.dimension!=m2.dimension) return Matrix();
	Matrix RES(m1, m2);
...
Matrix operator * (const Matrix &m1, const Matrix &m2)
{
	if(m1.dimension.ss!=m2.dimension.ff) return Matrix();
	Matrix RES(m1, m2);
...
Matrix multiplyMatrix(const Matrix &m1, const Matrix &m2, LL mod=(1LL<<32))
{
...
Matrix bigModMatrix(Matrix m, LL p, LL mod=(1LL<<32))
{
...
int main()
{
	...
	TC(i, 1, tc){
		...
		I = multiplyMatrix(bigModMatrix(I, k, 10), m, 10);
